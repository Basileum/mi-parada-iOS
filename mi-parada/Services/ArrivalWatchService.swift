...
class ArrivalWatchService {
    ...
    func stopWatchRequest(watchRequest: WatchRequest) {
        logger.info("ArrivalWatchService: Stopping watch request for stop \(watchRequest.stopId) line \(watchRequest.line)")
        guard let baseURL = Bundle.main.infoDictionary?["API_BASE_URL"] as? String,
              let url = URL(string: baseURL + "/arrival-watch/liveactivity/stop") else {
            logger.error("ArrivalWatchService: Invalid URL")
            return
        }
